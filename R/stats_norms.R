...
#' @section Missing Data: The algorithm cannot handle missing data. The
...
#' @param mfs_colname The column name of the variable containing the magnetic
#'    field strength codes. Either 0L (3T) *or* 1L (1.5T).
#' @param manufid_colname The column name of the variable containing the
...
norm_statsdf <- function(statsdf,
  ...
  for (i in c(etiv_colname, age_colname, gender_colname, id_colname,
              mfs_colname, manufid_colname)) {
    if (!i %in% colnames(statsdf)) {
      stop(paste0("Column not found in statsdf: ", i))
    }
  }

  if (!all(complete.cases(statsdf[, c(etiv_colname,
                                      age_colname,
                                      gender_colname,
                                      mfs_colname,
                                      manufid_colname)]))) {
    stop("no missing/NA values allowed.")
  }
             ...
             function(i) {
               do.call(
                 ...
                 args = list(
                   ...
                   magnetic_field_strength = statsdf[i, mfs_colname],
                   modality_manuf_id = statsdf[i, manufid_colname],
                   normstab = normstab
                 )
               )
             },
